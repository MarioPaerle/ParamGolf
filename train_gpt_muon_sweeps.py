...
class Hyperparameters:
    ...
    # NS schedule: "constant" | "linear_up" | "linear_down" | "cosine" | "step" | "warmup"
    ns_schedule = os.environ.get("NS_SCHEDULE", "constant")
    ns_steps_min = int(os.environ.get("NS_STEPS_MIN", 2))
    ns_steps_max = int(os.environ.get("NS_STEPS_MAX", 10))
    # Training step at which the schedule completes (reaches max/min). 0 = use ITERATIONS.
    ns_schedule_steps = int(os.environ.get("NS_SCHEDULE_STEPS", 0))
    # For "step" schedule: comma-separated step:value pairs e.g. "0:3,2000:5,6000:8"
    ns_step_milestones = os.environ.get("NS_STEP_MILESTONES", "")
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    ...
    return torch.tanh(10 * G.bfloat16())
...
def compute_ns_steps(args, step: int) -> int:
    """Compute the number of Newton-Schulz iterations for the current training step."""
    schedule = args.ns_schedule
    lo, hi = args.ns_steps_min, args.ns_steps_max
    total = args.ns_schedule_steps if args.ns_schedule_steps > 0 else args.iterations

    if schedule == "constant":
        return args.muon_backend_steps

    frac = min(step / max(total - 1, 1), 1.0)

    if schedule == "linear_up":
        return int(round(lo + (hi - lo) * frac))

    if schedule == "linear_down":
        return int(round(hi - (hi - lo) * frac))

    if schedule == "cosine":
        return int(round(lo + (hi - lo) * 0.5 * (1.0 + math.cos(math.pi * frac))))

    if schedule == "warmup":
        warmup_frac = min(frac / 0.2, 1.0)
        return int(round(lo + (hi - lo) * warmup_frac))

    if schedule == "step":
        milestones = []
        for pair in args.ns_step_milestones.split(","):
            pair = pair.strip()
            if ":" in pair:
                s, v = pair.split(":", 1)
                milestones.append((int(s), int(v)))
        milestones.sort()
        val = args.muon_backend_steps
        for s, v in milestones:
            if step >= s:
                val = v
        return val
...
class Block(nn.Module):
    def __init__(
        ...
        self.ipsmear = SmearGate2(dim + dim // 2)
    ...
    def forward(
        ...
        if xss is not None:
            xss_cat = self.ipsmear(torch.cat(xss, dim=-1))
            xss_smeared = xss_cat.chunk(len(xss), dim=-1)
            for p in range(len(xss)):
                x_in = x_in + mix[2 + p] * xss_smeared[p]
...
def main() -> None:
    ...
    while True:
        ...
        ns_steps_now = compute_ns_steps(args, step)
        for group in optimizer_muon.param_groups:
            group["backend_steps"] = ns_steps_now
        optimizer_muon.step()
        zero_grad_all()
        # EMA update
        if args.ema_enabled:

...
class Hyperparameters:
    ...
    # --- Training schedule (modded-nanogpt style) ---
    # TRAIN_SCHEDULE: comma-separated "step:batch_tokens:seq_len" entries.
    # Controls both batch size and sequence length ramp during training.
    # E.g. "0:262144:128,500:262144:1024,1000:524288:2048,2000:786432:2048"
...
class TrainingStage:
    __slots__ = ("step", "batch_tokens", "seq_len", "lr_mul")
    def __init__(self, step: int, batch_tokens: int, seq_len: int, lr_mul: float):
        self.step = step
        self.batch_tokens = batch_tokens
        self.seq_len = seq_len
        self.lr_mul = lr_mul
    def __repr__(self) -> str:
        return f"({self.step}, batch={self.batch_tokens}, seq={self.seq_len}, lr={self.lr_mul:.3f})"

def _build_train_stages(args: Hyperparameters) -> list[TrainingStage]:
    """Parse TRAIN_SCHEDULE into sorted list of TrainingStage.

    Format: "step:batch_tokens:seq_len,..."
    LR multiplier follows sqrt scaling on batch size: lr_mul = sqrt(cur/final).
    Falls back to single stage with TRAIN_BATCH_TOKENS / TRAIN_SEQ_LEN if empty.
    """
    if not args.train_schedule:
        return [TrainingStage(0, args.train_batch_tokens, args.train_seq_len, 1.0)]
    raw = []
    for entry in args.train_schedule.split(","):
        parts = entry.strip().split(":")
        raw.append((int(parts[0]), int(parts[1]), int(parts[2])))
    raw.sort(key=lambda x: x[0])
    final_batch = raw[-1][1]
    return [
        TrainingStage(s, bt, sl,
                      math.sqrt(bt / final_batch) if args.train_schedule_lr_scale else 1.0)
        for s, bt, sl in raw
    ]

def current_stage(step: int, stages: list[TrainingStage]) -> TrainingStage:
    """Return the active TrainingStage for the given step."""
    for i in range(len(stages) - 1, -1, -1):
        if step >= stages[i].step:
            return stages[i]
    return stages[0]
...
def main() -> None:
    ...
    log0(
        ...
        f"train_schedule:{args.train_schedule or 'none'}"
    )
    ...
    train_stages = _build_train_stages(args)
    if len(train_stages) > 1:
        log0(f"train_schedule:stages {train_stages}")
    if args.warmup_steps > 0 or len(train_stages) > 1:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        # Pre-compile all (batch, seq_len) shapes to avoid recompilation stalls
        # mid-training. Enable QAT during precompile so late_qat doesn't trigger
        # inference-tensor errors on recompile.
        if len(train_stages) > 1:
            import torch._dynamo as dynamo
            dynamo.config.recompile_limit = 64
            saved_qat = CastedLinear._qat_enabled
            if args.late_qat:
                CastedLinear._qat_enabled = True
            seen_shapes: set[tuple[int, int]] = set()
            for stage_idx, stage in enumerate(train_stages):
                local_tokens = stage.batch_tokens // (world_size * grad_accum_steps)
                local_seqs = local_tokens // stage.seq_len
                shape_key = (local_seqs, stage.seq_len)
                if shape_key in seen_shapes:
                    continue
                seen_shapes.add(shape_key)
                zero_grad_all()
                for micro_step in range(grad_accum_steps):
                    x, y = train_loader.next_batch(stage.batch_tokens, stage.seq_len, grad_accum_steps)
                    with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                        compile_loss = model(x, y)
                    (compile_loss * grad_scale).backward()
                zero_grad_all()
                log0(f"precompile:stage_{stage_idx} batch={stage.batch_tokens} seq_len={stage.seq_len} local_seqs={local_seqs}")
            CastedLinear._qat_enabled = saved_qat
        # Standard warmup (optimizer scouting)
    ...
    while True:
        ...
        cur = current_stage(step, train_stages)
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(cur.batch_tokens, cur.seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
        ...
        for opt in optimizers:
            for group in opt.param_groups:
                group["lr"] = group["base_lr"] * scale * cur.lr_mul
        ...
        if should_log_train:
            sched_info = f" batch:{cur.batch_tokens} seq:{cur.seq_len}" if len(train_stages) > 1 else ""
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms{sched_info}"
            )

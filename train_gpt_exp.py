...
class GPT(nn.Module):
    def __init__(
        ...
        self.tok_emb = nn.Embedding(vocab_size, model_dim * 2)
        self.se_gate = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        se_w = self.tok_emb.weight[:, d:]
        z = F.linear(x, se_w)
        # x = x + self.se_gate * (z @ self.tok_emb.weight[:, :d])
        ...
        if self.tie_embeddings:
            logits_proj = F.linear(x_flat, self.tok_emb.weight[:, :d])
        else:
        ...
        if True and self.training:
            mid_loss = F.cross_entropy(z.reshape(-1, z.size(-1)).float(), targets, reduction="mean")
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        # Secondary embedding feedback (no loss at eval)
        se_w = self.tok_emb.weight[:, d:]
        z = F.linear(x, se_w)
        # x = x + gate_val * (z @ self.tok_emb.weight[:, :d])
        for i in range(self.num_decoder_layers):
            bi = self.num_encoder_layers + i
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
        ...
        x = self.final_norm(x)
        if self.tie_embeddings:
            logits_proj = F.linear(x, self.tok_emb.weight[:, :d])
        else:
...
def main() -> None:
    ...
    scalar_params.append(base_model.se_gate)

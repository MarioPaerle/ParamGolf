...
class Hyperparameters:
    ...
    # SEQ_SCHEDULE: comma-separated "step:seq_len" pairs, e.g. "0:128,400:512,800:1024,1200:2048"
    # Falls back to legacy SEQ_WARMUP_STEPS/SEQ_WARMUP_MIN if not set.
    seq_schedule = os.environ.get("SEQ_SCHEDULE", "0:128,400:512,800:1024,1200:2048")
    seq_warmup_steps = int(os.environ.get("SEQ_WARMUP_STEPS", 800))
    seq_warmup_min = int(os.environ.get("SEQ_WARMUP_MIN", 128))
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self.smear(x)  # Restored perfectly
...
# --- Sequence length scheduling (modded-nanogpt style) ---
# Two discrete stages: short seqs for first N steps, then full length.
# Pre-compiled in warmup so no recompilation cost during timed training.

def _build_seq_stages(args: Hyperparameters) -> list[tuple[int, int]]:
    """Returns [(transition_step, seq_len), ...] sorted by step.

    Supports SEQ_SCHEDULE env var for multi-stage schedules, e.g.:
        SEQ_SCHEDULE="0:128,400:512,800:1024,1200:2048"
    Falls back to legacy two-stage SEQ_WARMUP_STEPS/SEQ_WARMUP_MIN behaviour.
    """
    if args.seq_schedule:
        stages = []
        for entry in args.seq_schedule.split(","):
            step_str, sl_str = entry.strip().split(":")
            stages.append((int(step_str), int(sl_str)))
        stages.sort(key=lambda x: x[0])
        return stages
    # Legacy two-stage fallback
    if args.seq_warmup_steps <= 0 or args.seq_warmup_min >= args.train_seq_len:
        return [(0, args.train_seq_len)]
    return [
        (0, args.seq_warmup_min),
        (args.seq_warmup_steps, args.train_seq_len),
    ]

def scheduled_seq_len(step: int, stages: list[tuple[int, int]]) -> int:
...
def main() -> None:
    ...
    seq_stages = _build_seq_stages(args)
    if len(seq_stages) > 1:
        log0(f"seq_schedule:stages {[(s, sl) for s, sl in seq_stages]}")
    if args.warmup_steps > 0 or len(seq_stages) > 1:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        # Pre-compile all stage shapes (untimed, a la modded-nanogpt)
        # Enable QAT during precompile so recompilation at seq_len transitions
        # doesn't hit "inference tensors cannot be saved for backward" errors.
        if len(seq_stages) > 1:
            ...
            for stage_idx, (_, sl) in enumerate(seq_stages):
                zero_grad_all()
                ...
                zero_grad_all()
                log0(f"precompile:stage_{stage_idx} seq_len={sl}")
    ...
    while True:
        ...
        if args.late_qat and args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled and step > 10:
            CastedLinear._qat_enabled = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        cur_seq_len = scheduled_seq_len(step, seq_stages)
        zero_grad_all()

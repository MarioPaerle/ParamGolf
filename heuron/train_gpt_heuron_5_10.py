...
class GPT(nn.Module):
    def __init__(
        ...
        # XSI v3 per-block learnable scales applied at every block exit (against x0).
        self.xsi_alpha_blocks = nn.Parameter(torch.zeros(num_layers, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        x0_n_blk = F.normalize(x0.detach(), dim=-1)
        for i in range(self.num_encoder_layers):
            ...
            if v0 is None and raw_v is not None:
                v0 = raw_v
            a = torch.tanh(self.xsi_alpha_blocks[i]).to(dtype=x.dtype)
            x = x - a * (x * x0_n_blk).sum(dim=-1, keepdim=True) * x0_n_blk
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
            a = torch.tanh(self.xsi_alpha_blocks[bi]).to(dtype=x.dtype)
            x = x - a * (x * x0_n_blk).sum(dim=-1, keepdim=True) * x0_n_blk
...
def main() -> None:
    ...
    scalar_params.append(base_model.xsi_alpha_blocks)

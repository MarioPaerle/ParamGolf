...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mlp_gate,me_layer_scale",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Input Projections: project x0 into alternative representations for per-layer injection
        # Instead of separate embedding tables, we project the initial embedding x0
        self.me_dim = me_dim if me_enabled else 0
        if me_enabled and me_num_extra > 0:
            self.me_embeds = nn.ModuleList()  # unused in IP mode, keep for compat
            # Projection matrices: x0 (model_dim) -> me_dim
            self.ip_projs = nn.ParameterList([
                nn.Parameter(torch.zeros(me_dim, model_dim, dtype=torch.float32))
                for _ in range(me_num_extra)
            ])
            for p in self.ip_projs:
                nn.init.orthogonal_(p, gain=0.01)  # small init so starts near zero
            # Per-layer scales, zero-init
    ...
    def _compute_me_signals(self, input_ids: Tensor, model_dim: int, x0: Tensor | None = None) -> list[Tensor]:
        """Precompute signals: embeddings from input_ids or projections from x0."""
        signals = []
        if hasattr(self, 'ip_projs') and len(self.ip_projs) > 0 and x0 is not None:
            # Input Projections mode: project x0 into alternative representations
            for proj in self.ip_projs:
                e = F.linear(x0, proj.to(dtype=x0.dtype))  # (B, T, me_dim)
                signals.append(F.pad(e, (0, model_dim - self.me_dim)))  # (B, T, model_dim)
        else:
            # Standard ME mode: use embedding tables
    ...
    def _inject_me(self, x: Tensor, layer_idx: int, me_signals: list[Tensor]) -> Tensor:
        """Inject multiple embedding signals at layer input."""
        if not me_signals:
            return x
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        me_signals = self._compute_me_signals(input_ids, x.size(-1), x0=x0)
...
def main() -> None:
    ...
    # Input Projections: proj weights -> scalar (small matrices), layer scales -> scalar
    for me_emb in base_model.me_embeds:
        tok_params.append({"params": [me_emb.weight], "lr": token_lr, "base_lr": token_lr})
    if hasattr(base_model, 'ip_projs'):
        for p in base_model.ip_projs:

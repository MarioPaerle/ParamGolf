...
class GPT(nn.Module):
    def __init__(
        ...
        self.block_dense_enc_logits = nn.Parameter(
            torch.full((self.num_encoder_layers, self.num_encoder_layers), -6.0, dtype=torch.float32)
        )
        self.block_dense_dec_logits = nn.Parameter(
            torch.full((self.num_decoder_layers, self.num_decoder_layers), -6.0, dtype=torch.float32)
        )

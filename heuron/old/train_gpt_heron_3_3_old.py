...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,dense_logits,dense_gate_w",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Per-pair sigmoid-bounded weights AND a per-target-layer input-dependent gate.
        # dense_logits[i, j] -> sigmoid weight for layer i reading layer j (j < i), init -6.0 -> ~0.0025
        # dense_gate_w[i, :] -> 12-dim gate input (zero init, 2*sigmoid(0)=1.0 starts as no-op multiplier)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            if i > 0:
                ...
                # Per-target-layer input-dependent gate from x[:12] -> scalar per token
                g = 2.0 * torch.sigmoid(F.linear(x[..., :12], gate_w[i:i+1]))  # (B, T, 1)
                x = x + g * contribution
        ...
        for i in range(self.num_decoder_layers):
            ...
            stacked = torch.stack(layer_outputs, dim=0)  # (bi, B, T, D)
            w = dense_w[bi, :bi]
            contribution = torch.einsum('i,ibtd->btd', w, stacked)

...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mlp_gate,me_layer_scale,me_gate_w,block_dense_enc_logits,block_dense_dec_logits",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Encoder block & decoder block dense connections, sigmoid-bounded.
        # Ablation: stronger init at -3.0 (sigmoid(-3) = 0.0474, ~20x stronger than -6.0).
        self.block_dense_enc_logits = nn.Parameter(
            torch.full((self.num_encoder_layers, self.num_encoder_layers), -3.0, dtype=torch.float32)
        )
        self.block_dense_dec_logits = nn.Parameter(
            torch.full((self.num_decoder_layers, self.num_decoder_layers), -3.0, dtype=torch.float32)
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            if i > 0:
                stacked = torch.stack(enc_outputs, dim=0)
                w = enc_w[i, :i]
...
def main() -> None:
    ...
    scalar_params.append(base_model.block_dense_enc_logits)
    scalar_params.append(base_model.block_dense_dec_logits)

...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mlp_gate,me_layer_scale,me_gate_w,dense_logits",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        if me_enabled and me_num_extra > 0:
            ...
            self.me_layer_scales = nn.ParameterList()
            self.me_gate_ws = nn.ParameterList()
        # Real Attention Residual (DenseFormer-style):
        # Per-pair sigmoid-bounded weights, init at -6.0 (~0.0025 contribution at start).

...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mlp_gate,me_layer_scale,me_gate_w,dense_logits,dense_gate_w",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Per-pair sigmoid weights + per-target-layer input-dependent gate
        self.dense_logits = nn.Parameter(torch.full((num_layers, num_layers), -6.0, dtype=torch.float32))
        self.dense_gate_w = nn.Parameter(torch.zeros(num_layers, 12, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        gate_w = self.dense_gate_w.to(dtype=x.dtype)
        for i in range(self.num_encoder_layers):
            x = self._inject_me(x, i, me_signals)
            if i > 0:
                stacked = torch.stack(layer_outputs, dim=0)
                w = dense_w[i, :i]
                contribution = torch.einsum('i,ibtd->btd', w, stacked)
                g = 2.0 * torch.sigmoid(F.linear(x[..., :12], gate_w[i:i+1]))
                x = x + g * contribution
        ...
        for i in range(self.num_decoder_layers):
            ...
            w = dense_w[bi, :bi]
            contribution = torch.einsum('i,ibtd->btd', w, stacked)
            g = 2.0 * torch.sigmoid(F.linear(x[..., :12], gate_w[bi:bi+1]))
            x = x + g * contribution
...
def main() -> None:
    ...
    for s in base_model.me_gate_ws:
        scalar_params.append(s)
    scalar_params.append(base_model.dense_logits)
    scalar_params.append(base_model.dense_gate_w)

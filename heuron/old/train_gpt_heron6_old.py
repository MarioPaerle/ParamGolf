...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gate,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mlp_gate",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Input-dependent skip gate: per decoder layer, reads 12 dims -> scalar per token
        self.skip_gate_w = nn.Parameter(torch.zeros(self.num_skip_weights, 12, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            if skips:
                skip_g = 2.0 * torch.sigmoid(F.linear(x[..., :12], self.skip_gate_w[i:i+1].to(dtype=x.dtype)))
                x = x + skip_g * self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
...
def main() -> None:
    ...
    if base_model.skip_weights.numel() > 0:
        ...
        scalar_params.append(base_model.skip_gate_w)

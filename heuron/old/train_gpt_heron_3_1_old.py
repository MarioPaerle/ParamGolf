...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # DenseFormer: track all layer outputs (encoder + decoder) for dense connections
        layer_outputs: list[Tensor] = []
        dense_w = torch.sigmoid(self.dense_logits).to(dtype=x.dtype)
        for i in range(self.num_encoder_layers):
            if i > 0:
                # Sum of weighted previous outputs (i previous layers)
        ...
        for i in range(self.num_decoder_layers):
            ...
            # Dense from all previous layer outputs (encoder + decoder so far)

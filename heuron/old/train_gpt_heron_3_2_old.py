...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,block_dense_enc_logits,block_dense_dec_logits",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Encoder block (layers 0..num_encoder_layers-1) reads only previous encoder layers
        # Decoder block (layers num_encoder_layers..num_layers-1) reads only previous decoder layers
        # Sigmoid-bounded weights init at -6.0 -> ~0.0025 contribution at start.
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        enc_outputs: list[Tensor] = []  # Block AttnRes: encoder block outputs
        dec_outputs: list[Tensor] = []  # Block AttnRes: decoder block outputs

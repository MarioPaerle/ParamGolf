...
class Hyperparameters:
    ...
    me_bigram_vocab = int(os.environ.get("ME_BIGRAM_VOCAB", 1536))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mlp_gate,me_layer_scale,me_gate_w",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # Lightweight input-dependent attention gate: reads first 12 dims, per-head output
        # Zero-init -> sigmoid(0)=0.5, scaled by 2 -> starts at 1.0 (no-op)
        self.attn_gate_w = nn.Parameter(torch.zeros(num_heads, 12, dtype=torch.float32))
        self.gated_attention = False  # replaced by lightweight gate above
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # Lightweight per-head attention gate from first 12 dims of input
...
class SmearGate(nn.Module):
    def __init__(self, dim: int):
        ...
        # Input-dependent smear modulation: reads first 12 dims, per-token scalar
        # Zero-init -> 2*sigmoid(0)=1.0, so at init g_input=1.0 (no change to base behavior)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        g = g_base * g_input  # per-dim base * per-token input-dependent modulation
...
class Block(nn.Module):
    def __init__(
        ...
        # Lightweight input-dependent MLP gate: reads first 12 dims, outputs scalar per token
...
class GPT(nn.Module):
    def __init__(
        ...
        me_dim: int = 128,
        me_num_extra: int = 1,
        me_bigram_vocab: int = 0,
    ):
        super().__init__()
        self.me_bigram_vocab = me_bigram_vocab
        ...
        # Multiple Embeddings with input-dependent gating
        # Gate reads first 12 dims of both x and me_signal to produce per-token scalar
        self.me_dim = me_dim if me_enabled else 0
        if me_enabled and me_num_extra > 0:
            me_vocab = me_bigram_vocab if me_bigram_vocab > 0 else vocab_size
            self.me_embeds = nn.ModuleList([
                nn.Embedding(me_vocab, me_dim) for _ in range(me_num_extra)
            ])
            for emb in self.me_embeds:
                nn.init.normal_(emb.weight, std=0.01)
            # Per-layer scales (base magnitude, zero-init)
    ...
    def _me_bigram_hash(self, tokens: Tensor) -> Tensor:
        """Bigram hash for ME: hash(current_token, prev_token) -> index into bigram vocab.
        Same scheme as BigramHashEmbedding but for ME indices."""
        t = tokens.to(torch.int32)
        mod = self.me_bigram_vocab - 1
        out = torch.empty_like(t)
        out[..., 0] = mod  # first position has no prev token
        out[..., 1:] = torch.bitwise_xor(36313 * t[..., 1:], 27191 * t[..., :-1]) % mod
        return out.long()
    def _compute_me_signals(self, input_ids: Tensor, model_dim: int) -> list[Tensor]:
        ...
        signals = []
        if self.me_bigram_vocab > 0:
            me_ids = self._me_bigram_hash(input_ids)
        else:
            me_ids = input_ids
        for emb in self.me_embeds:
            e = emb(me_ids)  # (B, T, me_dim)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        me_bigram_vocab=args.me_bigram_vocab,
    ).to(device).bfloat16()
    ...
    scalar_params.append(base_model.smear.smear_input_w)
    ...
    # Multiple Embeddings: embed weights -> Adam (tok), layer scales + gate weights -> scalar
    ...
    for s in base_model.me_layer_scales:
        scalar_params.append(s)
    for s in base_model.me_gate_ws:
        scalar_params.append(s)
    ...
    ema_state = {name: t.detach().float().clone() for name, t in base_model.state_dict().items()} if args.ema_enabled else {}
    ...
    eval_model = GPT(
        ...
        me_enabled=args.me_enabled, me_dim=args.me_dim, me_num_extra=args.me_num_extra, me_bigram_vocab=args.me_bigram_vocab,
    ).to(device).bfloat16()

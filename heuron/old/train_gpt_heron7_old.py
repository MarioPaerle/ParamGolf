...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,mlp_gate",
    ).split(",")
    if pattern
)
...
class Block(nn.Module):
    def __init__(
        ...
        # Per-dim input-dependent MLP gate: reads first 12 dims, gates each output dim independently
        # Zero-init -> 2*sigmoid(0)=1.0 per dim (no-op). 6144 params per layer, still tiny.
        self.mlp_gate_w = nn.Parameter(torch.zeros(dim, 12, dtype=torch.float32))
        if dtg:

...
class Hyperparameters:
    ...
    me_hash_vocab = int(os.environ.get("ME_HASH_VOCAB", 512))
...
class GPT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.me_hash_vocab = me_hash_vocab
        ...
        if me_enabled and me_num_extra > 0:
            me_vocab = me_hash_vocab if me_hash_vocab > 0 else vocab_size
    ...
    def _compute_me_signals(self, input_ids: Tensor, model_dim: int) -> list[Tensor]:
        ...
        signals = []
        if self.me_hash_vocab > 0:
            me_ids = (input_ids.to(torch.int32) * 36313) % self.me_hash_vocab
            me_ids = me_ids.long()
        else:
            me_ids = input_ids
        for emb in self.me_embeds:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        me_hash_vocab=args.me_hash_vocab,
    ).to(device).bfloat16()
    ...
    eval_model = GPT(
        ...
        me_enabled=args.me_enabled, me_dim=args.me_dim, me_num_extra=args.me_num_extra, me_hash_vocab=args.me_hash_vocab,
    ).to(device).bfloat16()

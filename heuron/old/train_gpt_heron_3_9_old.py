...
# Eager-mode dense add: forces a graph break so inductor cannot fuse the dense
# stack/einsum into rms_norm (which OOMs the triton kernel without skips).
@torch.compiler.disable(recursive=False)
def _eager_dense_add(x: Tensor, layer_outputs_list: list, dense_w_row: Tensor) -> Tensor:
    stacked = torch.stack(layer_outputs_list, dim=0)
    return x + torch.einsum('i,ibtd->btd', dense_w_row.to(dtype=x.dtype), stacked)
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,dense_logits",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Per-pair sigmoid-bounded weights. dense_logits[i, j] = logit for layer i reading layer j (j < i).
        # Init at -6.0 -> sigmoid(-6) ~= 0.0025, so initial dense contribution is essentially zero.
        # Bounded by sigmoid (max contribution per pair is 1.0) so cannot blow up the residual stream.
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x0 = x
        v0 = None
        # B3-9: U-Net skips REMOVED. Only DenseFormer connections remain. Tests if dense
        # works when not redundant with skips. Dense add runs in eager mode (compile.disable)
        # to break the over-fusion that OOMs the triton kernel without skips.
        ve_cache: dict = {}
        layer_outputs: list[Tensor] = []
        dense_w = torch.sigmoid(self.dense_logits)
        for i in range(self.num_encoder_layers):
            if i > 0:
                x = _eager_dense_add(x, layer_outputs, dense_w[i, :i])
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = _eager_dense_add(x, layer_outputs, dense_w[bi, :bi])

...
class GPT(nn.Module):
    def __init__(
        ...
        self.xsi_alpha_mid = nn.Parameter(torch.zeros(1, dtype=torch.float32))  # 5_18 mid-network XSI
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # 5_11: use final_norm(x0) as projection direction (RMS-normalized) to fix magnitude mismatch with x
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve, v0=v0)
        # XSI v3 (final-logit XSI): mirror of training forward() — 5_11 final_norm target
        x0_n = F.normalize(self.final_norm(x0), dim=-1)
        xsi_a = torch.tanh(self.xsi_alpha).to(dtype=x.dtype)
        x = x - xsi_a * (x * x0_n).sum(dim=-1, keepdim=True) * x0_n
        x = self.final_norm(x)
...
class _HessianGPT(nn.Module):
    ...
    def forward(self, input_ids, target_ids):
        ...
        # XSI v3 (final-logit XSI): must mirror GPT.forward so calibration matches — 5_11 final_norm target
        x0_n = F.normalize(self.final_norm(x0), dim=-1)
        xsi_a = torch.tanh(self.xsi_alpha).to(dtype=x.dtype)
        x = x - xsi_a * (x * x0_n).sum(dim=-1, keepdim=True) * x0_n
        x = self.final_norm(x)
...
def main() -> None:
    ...
    scalar_params.append(base_model.xsi_alpha_mid)

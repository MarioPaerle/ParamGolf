...
# FA3 (flash_attn_interface.flash_attn_func) removed: not installable on Cineca A100s.
# F.scaled_dot_product_attention with enable_flash_sdp(True) dispatches to FlashAttention-2 on A100.
class Hyperparameters:
    ...
    warmdown_iters = int(os.environ.get("WARMDOWN_ITERS", 218)) # Changed to from 3500 on 8xH100 10 minutes
    ...
    # Battle 2 transferred: Multiple Embeddings (gated ME)
    me_enabled = bool(int(os.environ.get("ME_ENABLED", "1")))
    me_dim = int(os.environ.get("ME_DIM", 128))
    me_num_extra = int(os.environ.get("ME_NUM_EXTRA", 1))
    # GPTQ calibration
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,me_layer_scales,me_gate_ws,attn_gate_w,mlp_gate_w,mlp_gate,smear_input_w",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # Battle 1: lightweight per-head input-dependent attention gate
        # Zero-init -> 2*sigmoid(0)=1.0 (no-op start). (num_heads * 12) params per layer.
        self.attn_gate_w = nn.Parameter(torch.zeros(num_heads, 12, dtype=torch.float32))
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # FA3 -> SDPA(FA2 backend on A100). FA3 takes (B,T,H,D); SDPA takes (B,H,T,D).
        ...
        y = y.transpose(1, 2).contiguous()  # back to (B,T,H,D) for XSA / reshape
        if self.use_xsa:
        ...
        if self.gated_attention:
            ...
            y = y * gate
        # Battle 1 lightweight per-head attention gate from first 12 dims of input.
        # Init at no-op (2*sigmoid(0)=1.0). (B,T,num_heads) -> (B,T,num_heads,1).
        attn_g = 2.0 * torch.sigmoid(F.linear(x[..., :12], self.attn_gate_w.to(dtype=y.dtype))).unsqueeze(-1)
        y = y * attn_g
...
class SmearGate(nn.Module):
    def __init__(self, dim: int):
        ...
        # Battle 1: input-dependent smear modulation. zero-init -> 2*sigmoid(0)=1.0 (no-op).
        self.smear_input_w = nn.Parameter(torch.zeros(1, 12, dtype=torch.float32))
    def forward(self, x: Tensor) -> Tensor:
        g_base = torch.sigmoid(self.gate.to(dtype=x.dtype))[None, None, :]
        g_input = 2.0 * torch.sigmoid(F.linear(x[..., :12], self.smear_input_w.to(dtype=x.dtype)))
        g = g_base * g_input
...
class Block(nn.Module):
    def __init__(
        ...
        # Battle 1: lightweight per-token MLP output gate. zero-init -> 2*sigmoid(0)=1.0 (no-op).
        self.mlp_gate_w = nn.Parameter(torch.zeros(1, 12, dtype=torch.float32))
        if dtg:
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # Battle 1 lightweight MLP output gate: per-token scalar from first 12 dims of x_out
        mlp_g = 2.0 * torch.sigmoid(F.linear(x_out[..., :12], self.mlp_gate_w.to(dtype=x_out.dtype)))
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * mlp_g * mlp_out
...
class GPT(nn.Module):
    def __init__(
        ...
        # Battle 2 transferred: Multiple Embeddings (gated)
        self.me_dim = me_dim if me_enabled else 0
        if me_enabled and me_num_extra > 0:
            self.me_embeds = nn.ModuleList([
                nn.Embedding(vocab_size, me_dim) for _ in range(me_num_extra)
            ])
            for emb in self.me_embeds:
                nn.init.normal_(emb.weight, std=0.01)
            # Per-layer base scales (zero-init)
            self.me_layer_scales = nn.ParameterList([
                nn.Parameter(torch.zeros(me_num_extra, dtype=torch.float32))
                for _ in range(num_layers)
            ])
            # Per-layer input-dependent gate: reads cat(x[:12], me[:12]) -> scalar
            # Zero-init -> 2*sigmoid(0)=1.0 -> starts as no-op (same as ungated ME)
            self.me_gate_ws = nn.ParameterList([
                nn.Parameter(torch.zeros(me_num_extra, 24, dtype=torch.float32))
                for _ in range(num_layers)
            ])
        else:
            self.me_embeds = nn.ModuleList()
    ...
    def _compute_me_signals(self, input_ids: Tensor, model_dim: int) -> list[Tensor]:
        """Precompute multiple embedding signals, padded to model_dim."""
        signals = []
        for emb in self.me_embeds:
            e = emb(input_ids)  # (B, T, me_dim)
            signals.append(F.pad(e, (0, model_dim - self.me_dim)))  # (B, T, model_dim)
        return signals
    def _inject_me(self, x: Tensor, layer_idx: int, me_signals: list[Tensor]) -> Tensor:
        """Inject gated multiple embedding signals at layer input."""
        if not me_signals:
            return x
        scales = self.me_layer_scales[layer_idx].to(dtype=x.dtype)
        gate_w = self.me_gate_ws[layer_idx].to(dtype=x.dtype)
        for j, sig in enumerate(me_signals):
            # Gate reads first 12 dims of x and first 12 dims of me signal
            gate_input = torch.cat([x[..., :12], sig[..., :12]], dim=-1)  # (..., 24)
            g = 2.0 * torch.sigmoid(F.linear(gate_input, gate_w[j:j+1]))  # (..., 1)
            x = x + scales[j] * g * sig
        return x
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
        # XSI v3 (final-logit XSI): orthogonalize against x0 before final norm.
        x0_n = F.normalize(x0, dim=-1)
        x = x - (x * x0_n).sum(dim=-1, keepdim=True) * x0_n
        x = self.final_norm(x)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve, v0=v0)
        # XSI v3 (final-logit XSI): mirror of training forward()
        x0_n = F.normalize(x0, dim=-1)
        x = x - (x * x0_n).sum(dim=-1, keepdim=True) * x0_n
        x = self.final_norm(x)
        if self.tie_embeddings:
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
                 ...
                 me_enabled=False, me_dim=128, me_num_extra=0):
        super().__init__()
        self.tie_embeddings = tie_embeddings
        self.logit_softcap = logit_softcap
        self.num_layers = num_layers
        ...
        if self.ve_layer_indices:
            ...
            self.ve_shared = None
            self.ve_layer_scales = nn.ParameterList()
        # Battle 2 transferred: ME for Hessian-time activations to match training
        self.me_dim = me_dim if me_enabled else 0
        if me_enabled and me_num_extra > 0:
            self.me_embeds = nn.ModuleList([nn.Embedding(vocab_size, me_dim) for _ in range(me_num_extra)])
            self.me_layer_scales = nn.ParameterList([
                nn.Parameter(torch.zeros(me_num_extra, dtype=torch.float32)) for _ in range(num_layers)
            ])
            self.me_gate_ws = nn.ParameterList([
                nn.Parameter(torch.zeros(me_num_extra, 24, dtype=torch.float32)) for _ in range(num_layers)
            ])
        else:
            self.me_embeds = nn.ModuleList()
            self.me_layer_scales = nn.ParameterList()
    ...
    def _compute_me_signals(self, input_ids, model_dim):
        signals = []
        for emb in self.me_embeds:
            e = emb(input_ids)
            signals.append(F.pad(e, (0, model_dim - self.me_dim)))
        return signals
    def _inject_me(self, x, layer_idx, me_signals):
        ...
        for j, sig in enumerate(me_signals):
            ...
            g = 2.0 * torch.sigmoid(F.linear(gate_input, gate_w[j:j+1]))
            x = x + scales[j] * g * sig
        return x
    def forward(self, input_ids, target_ids):
        ...
        # XSI v3 (final-logit XSI): mirror for calibration
        x0_n = F.normalize(x0, dim=-1)
        x = x - (x * x0_n).sum(dim=-1, keepdim=True) * x0_n
        x = self.final_norm(x)
        x_flat = x.reshape(-1, x.size(-1))
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        me_enabled=args.me_enabled,
        me_dim=args.me_dim,
        me_num_extra=args.me_num_extra,
    ).to(device).bfloat16()
    ...
    scalar_params.append(base_model.smear.smear_input_w)  # Battle 1 lightweight smear input gate
    ...
    # Battle 2 transferred: ME embeddings -> tok_params; per-layer scales+gates -> scalar_params
    if len(base_model.me_embeds) > 0:
        for me_emb in base_model.me_embeds:
            tok_params.append({"params": [me_emb.weight], "lr": token_lr, "base_lr": token_lr})
        for s in base_model.me_layer_scales:
            scalar_params.append(s)
        for g in base_model.me_gate_ws:
    ...
    hessian_model = _HessianGPT(
        ...
        me_enabled=args.me_enabled, me_dim=args.me_dim, me_num_extra=args.me_num_extra,
    ).to(device).bfloat16()

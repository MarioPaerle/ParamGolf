...
class GPT(nn.Module):
    def __init__(
        ...
        # XSI v2 layerwise: learnable global α (init 0 → no-op = baseline behavior). Bounded via tanh.
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # XSI v2 (Layerwise k-XSI): every k-th layer's output is orthogonalized
        # against the residual-stream snapshot from k layers ago. Parameterless.
        # The projection target uses .detach() (stop-gradient) to prevent
        # torch.compile from fusing a gigantic backward kernel that exceeds
        # A100 SMEM (BYOL/SimSiam-style stop-grad — gradient still flows
        # through x naturally).
        xsi_k = 4
        saved_inputs: list = [None] * n
        for i in range(self.num_encoder_layers):
            saved_inputs[i] = x.detach()
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0)
            if (i + 1) % xsi_k == 0 and (i + 1) >= xsi_k:
                src = saved_inputs[i + 1 - xsi_k]
                src_n = F.normalize(src, dim=-1)
                xsi_a = torch.tanh(self.xsi_alpha).to(dtype=x.dtype)
                x = x - xsi_a * (x * src_n).sum(dim=-1, keepdim=True) * src_n
        ...
        for i in range(self.num_decoder_layers):
            ...
            saved_inputs[bi] = x.detach()
            ve = self._get_ve(bi, input_ids, ve_cache)
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve, v0=v0)
            if (bi + 1) % xsi_k == 0 and (bi + 1) >= xsi_k:
                src = saved_inputs[bi + 1 - xsi_k]
                src_n = F.normalize(src, dim=-1)
                xsi_a = torch.tanh(self.xsi_alpha).to(dtype=x.dtype)
                x = x - xsi_a * (x * src_n).sum(dim=-1, keepdim=True) * src_n
        x = self.final_norm(x)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        # XSI v2 (Layerwise k-XSI): mirror of training forward() (stop-grad on src)
        xsi_k = 4
        saved_inputs: list = [None] * n
        for i in range(self.num_encoder_layers):
            saved_inputs[i] = x.detach()
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
        ...
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        # XSI v2 layerwise: learnable global α (mirror of GPT.xsi_alpha for calibration)
        self.xsi_alpha = nn.Parameter(torch.zeros(1, dtype=torch.float32))
        self.blocks = nn.ModuleList([
    ...
    def forward(self, input_ids, target_ids):
        ...
        skips = []
        ve_cache = {}
        # XSI v2 (Layerwise k-XSI): must mirror GPT.forward for calibration
        xsi_k = 4
        saved_inputs = [None] * n
        for i in range(self.num_encoder_layers):
            saved_inputs[i] = x.detach()

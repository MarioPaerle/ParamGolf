...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # XSI v1 (Naive Block XSI, attn-only): orthogonalize attn_out against x_in.
        # Inline + .detach() ref to keep fused backward kernel under A100 SMEM (~167KB).
        # MLP-side XSI dropped to fit SMEM budget.
        _xsi_ref = F.normalize(x_in.detach(), dim=-1)
        attn_out = attn_out - (attn_out * _xsi_ref).sum(dim=-1, keepdim=True) * _xsi_ref
...
class _HessianBlock(nn.Module):
    ...
    def forward(self, x, x0, v_embed=None):
        ...
        # XSI v1 mirror for calibration (attn-only, inline)
        _xsi_ref = F.normalize(x_in.detach(), dim=-1)
        attn_out = attn_out - (attn_out * _xsi_ref).sum(dim=-1, keepdim=True) * _xsi_ref
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out

...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,x_mlp_gate_w,qgain_gate_w",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # 5_21: per-head lightweight attn output gate (Battle 1 pattern)
        self.attn_gate_w = nn.Parameter(torch.zeros(num_heads, 24, dtype=torch.float32))
        # 5_27: per-head Q-gain modulation gate (12->num_heads, multiplicative on q_gain)
        self.qgain_gate_w = nn.Parameter(torch.zeros(num_heads, 12, dtype=torch.float32))
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # 5_27: per-token, per-head q_gain modulation
        qg_mod = 2.0 * torch.sigmoid(F.linear(x[..., :12], self.qgain_gate_w.to(dtype=q.dtype)))
        q = q * (self.q_gain.to(dtype=q.dtype)[None, None, :] * qg_mod).unsqueeze(-1)
        ...
        # 5_24: per-head attn output gate from x[..., :24] (wider input)
        attn_g = 2.0 * torch.sigmoid(F.linear(x[..., :24], self.attn_gate_w.to(dtype=y.dtype))).unsqueeze(-1)
        y = y * attn_g
...
class Block(nn.Module):
    def __init__(
        ...
        # 5_15: cross-sublayer MLP gate — read from first 12 dims of attn_out (not x_out)
        self.x_mlp_gate_w = nn.Parameter(torch.zeros(1, 12, dtype=torch.float32))
        if dtg:
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # 5_15: gate the MLP contribution by attn_out's first 12 dims (cross-sublayer routing)
        mlp_g = 2.0 * torch.sigmoid(F.linear(attn_out[..., :12], self.x_mlp_gate_w.to(dtype=x_out.dtype)))
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * mlp_g * self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor, up_w, down_w)
...
class GPT(nn.Module):
    def __init__(
        ...
        # XSI v3 learnable scale (zero init → no-op at start). Bounded via tanh to [-1,1].
        self.xsi_alpha = nn.Parameter(torch.zeros(1, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
        # XSI v3 (final-logit XSI): orthogonalize the residual stream against the
        # initial input x0 before final norm + lm_head. The residual flow already
        # preserves x0 throughout, so the lm_head should predict NEW information
        # rather than echo current-token identity. Parameterless.
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
        ...
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        # XSI v3 learnable scale (mirror of GPT.xsi_alpha for calibration)
        self.xsi_alpha = nn.Parameter(torch.zeros(1, dtype=torch.float32))
        self.blocks = nn.ModuleList([
    ...
    def forward(self, input_ids, target_ids):
        ...
        # XSI v3 (final-logit XSI): must mirror GPT.forward so calibration matches
        x0_n = F.normalize(x0, dim=-1)
        xsi_a = torch.tanh(self.xsi_alpha).to(dtype=x.dtype)
        x = x - xsi_a * (x * x0_n).sum(dim=-1, keepdim=True) * x0_n
        x = self.final_norm(x)
...
def main() -> None:
    ...
    scalar_params.append(base_model.xsi_alpha)

...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,x_mlp_gate_w",
    ).split(",")
    if pattern
)
...
class Block(nn.Module):
    def __init__(
        ...
        self.x_mlp_gate_w = nn.Parameter(torch.zeros(1, 24, dtype=torch.float32))
        if dtg:
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # 5_26: cross-sublayer MLP gate widened — cat(x_in[..,:12], attn_out[..,:12])
        gate_in = torch.cat([x_in[..., :12], attn_out[..., :12]], dim=-1)
        mlp_g = 2.0 * torch.sigmoid(F.linear(gate_in, self.x_mlp_gate_w.to(dtype=x_out.dtype)))

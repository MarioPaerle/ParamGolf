...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.attn_gate_w = nn.Parameter(torch.zeros(num_heads, 64, dtype=torch.float32))
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # 5_24: per-head attn output gate from x[..., :64] (wider input)
        attn_g = 2.0 * torch.sigmoid(F.linear(x[..., :64], self.attn_gate_w.to(dtype=y.dtype))).unsqueeze(-1)
        y = y * attn_g

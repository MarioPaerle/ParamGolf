...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,xsi_alpha",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # XSI v3 per-channel learnable scale (zero init → no-op at start). Bounded via tanh.
        # Each output channel gets its own α, so the model can selectively keep/drop each
        # dimension of the x0 projection. model_dim=512 params (still ndim==1).
        self.xsi_alpha = nn.Parameter(torch.zeros(model_dim, dtype=torch.float32))
...
class _HessianGPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads,
        ...
        self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        # XSI v3 per-channel learnable scale (mirror of GPT.xsi_alpha for calibration)
        self.xsi_alpha = nn.Parameter(torch.zeros(model_dim, dtype=torch.float32))
        self.blocks = nn.ModuleList([

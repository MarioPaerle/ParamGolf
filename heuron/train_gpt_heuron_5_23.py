...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,x_mlp_gate_w,mlp_gate_w",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # 5_21: per-head attn output gate from x[..., :12]
...
class Block(nn.Module):
    def __init__(
        ...
        # 5_23: classic Battle 1 MLP gate (reads x_out[..,:12]) stacked on cross-sublayer one
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # 5_23: stack classic Battle 1 mlp_gate from x_out[..,:12]
        mlp_g2 = 2.0 * torch.sigmoid(F.linear(x_out[..., :12], self.mlp_gate_w.to(dtype=x_out.dtype)))
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * mlp_g * mlp_g2 * self.mlp(self.mlp_norm(x_out) * self.ln_scale_factor, up_w, down_w)

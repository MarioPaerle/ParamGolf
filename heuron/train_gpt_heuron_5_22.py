...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,x_mlp_gate_w,smear_input_w",
    ).split(",")
    if pattern
)
...
class SmearGate(nn.Module):
    def __init__(self, dim: int):
        ...
        # 5_22: Battle 1 smear input gate (12-dim, zero-init -> 1.0)
        self.smear_input_w = nn.Parameter(torch.zeros(1, 12, dtype=torch.float32))
    def forward(self, x: Tensor) -> Tensor:
        g = torch.sigmoid(self.gate.to(dtype=x.dtype))[None, None, :]
        x_prev = torch.cat([torch.zeros_like(x[:, :1]), x[:, :-1]], dim=1)
        out = (1 - g) * x + g * x_prev
        # 5_22: lightweight scalar input gate driven by first 12 dims
        g_in = 2.0 * torch.sigmoid(F.linear(x[..., :12], self.smear_input_w.to(dtype=x.dtype)))
        return out * g_in

...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.use_nov = False  # NOV: V := X (no value projection); K is broadcast to num_heads
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.use_nov:
            # NOV: V := X reshaped (no projection). K broadcast via expand+reshape (avoid repeat_interleave).
            v = x.reshape(bsz, seqlen, self.num_heads, self.head_dim).contiguous()
            if self.num_kv_heads != self.num_heads:
                group = self.num_heads // self.num_kv_heads
                k = k.unsqueeze(3).expand(bsz, seqlen, self.num_kv_heads, group, self.head_dim).reshape(bsz, seqlen, self.num_heads, self.head_dim).contiguous()
            raw_v = None
        else:
        ...
        y = F.scaled_dot_product_attention(
            ...
            attn_mask=None,
            is_causal=True,
            enable_gqa=(self.num_kv_heads != self.num_heads) and not self.use_nov,
        )
...
class GPT(nn.Module):
    def __init__(
        ...
        # 5_14 NOV: enable on the deepest 4 layers (substituting GQA with V=X)
        nov_last_n = int(os.environ.get("NOV_LAST_N", 4))
        if nov_last_n > 0:
            for i in range(max(0, num_layers - nov_last_n), num_layers):
                self.blocks[i].attn.use_nov = True
        self._init_weights()

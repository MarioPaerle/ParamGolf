...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,attn_gate_w,mlp_gate_w,mlp_gate,smear_input_w",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        # Battle 1: lightweight per-head input-dependent attention gate (zero-init -> 2*sigmoid(0)=1.0).
...
class Block(nn.Module):
    def __init__(
        ...
        # Battle 1 lightweight per-token MLP output gate.

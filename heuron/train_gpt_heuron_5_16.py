...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,bigram_gate_w",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bigram_gate_w = nn.Parameter(torch.zeros(1, 12, dtype=torch.float32))  # 5_16: gate bigram contribution per-token
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.bigram is not None:
            bg = self.bigram(input_ids)
            bg_g = 2.0 * torch.sigmoid(F.linear(x[..., :12], self.bigram_gate_w.to(dtype=bg.dtype)))
            x = x + bg_g * bg
...
def main() -> None:
    ...
    scalar_params.append(base_model.bigram_gate_w)
